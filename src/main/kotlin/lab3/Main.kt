...
//            val containersData = gson.fromJson(gson.toJson(it), ContainersData::class.java)
//            println(containersData.items)

            val containers = (jsonObject["containers"] as List<*>).map {
                val containersData = gson.fromJson(gson.toJson(it), ContainersData::class.java)
                println(containersData.items)
            when (containersData.type) {
//                "basicContainer" -> BasicContainer(id = containersData.id)
//                "heavyContainer" -> HeavyContainer(id = containersData.id)
//                "refrigeratedContainer" -> RefrigeratedContainer(id = containersData.id)
//                "liquidContainer" -> LiquidContainer(id = containersData.id)
